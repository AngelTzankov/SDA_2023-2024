...
който този елемент е минимален (най-нисък в хистограмата, т.е. образува правоъгълник с най-малка височина). 
Например за масива 3 2 4 3, за първия елемент отговорът е 3, тъй като няма друг правоъгълник, в който 3
да е минимален, освен този, който съдържа само първата 3-ка. 
За втория елемент това е целия масив и площта е 8, за 4-ката отговортът е 4 и за 
...
За да съставим този масив трябва да направим 2 прекалкулации:
 - Да намерим за всеки елемент първия индекс отляво на него, в който се съдържа по-малко число
...
Бележка: ако няма такъв индекс в l[i], записваме -1. Ако няма такъв в r[i], записваме n (големината на масива).
...
За да го създадем, обхождаме arr от началото, като всеки елемент добавяме в стек. Всеки следващ 
елемент, който обхождаме, премахва от върха на стека по-големите от него стойности. Когато намери 
елемент в стека, който е по-малък от него, значи сме стигнали до първия по-малък елемент в масива
отляво. Запазваме индекса на този елемент в l[i] и продължаваме. 
r[i] създаваме аналогично, като обхождаме масива отзад напред.
...
int main() {
    ...
    // Намираме максималното лице от всички лица на правоъгълници, където arr[i] е минимален елемент
    for(int i=0;i<n;++i) {
